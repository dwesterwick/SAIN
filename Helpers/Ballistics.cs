...
{
    // Credit to https://discussions.unity.com/t/projectile-trajectory-prediction/740303
    public static class Ballistics
    {
        // Note, doesn't take drag into account.

        public struct BallisticsPredictionResult
        {
            public Vector3[] Arc;
        ...
        public struct BallisticsPredictionInput
        {
            public Vector3 StartPosition;
            public Vector3 Direction;
            public float Velocity;
            public float PredictionResolution;
            public float MaxTime;
            public LayerMask RaycastMask;
            public bool DoRaycast;
        }

        public static BallisticsPredictionInput CreateData(Vector3 initialPosition, Vector3 direction, float velocity, float resolution, float maxTime, LayerMask mask, bool doRaycast)
        {
            return new BallisticsPredictionInput {
        ...
        public static BallisticsPredictionResult CalcTrajectory(BallisticsPredictionInput inputData)
        {
            Vector3[] arc = Ballistics.GetBallisticPath(inputData, out float timeOfFlight);
            return new BallisticsPredictionResult {
                Arc = arc,
                TimeOfFlight = timeOfFlight,
                HitPoint = inputData.DoRaycast ? Ballistics.GetHitPosition(arc, inputData.RaycastMask) : null,
            };
        }
        ...
        public static Vector3[] GetBallisticPath(out float timeOfFlight, Vector3 startPos, Vector3 forward, float velocity, float timeResolution, float maxTime = Mathf.Infinity)
        {
            timeOfFlight = Ballistics.GetTimeOfFlight(velocity, Vector3.Angle(forward, Vector3.up) * Mathf.Deg2Rad, startPos.y);
            maxTime = Mathf.Min(maxTime, timeOfFlight);
        ...
        public static Vector3[] GetBallisticPath(BallisticsPredictionInput inputData, out float timeOfFlight)
        {
            return GetBallisticPath(out timeOfFlight, inputData.StartPosition, inputData.Direction, inputData.Velocity, inputData.PredictionResolution, inputData.MaxTime);
        }
        ...
        public static RaycastHit? GetHitPosition(Vector3[] path, LayerMask lm)
        {
            RaycastHit hit;
            for (int i = 1; i < path.Length; i++) {
                //Debug.DrawRay (path [i - 1], path [i] - path [i - 1], Color.red, 10f);
                if (Physics.Raycast(path[i - 1], path[i] - path[i - 1], out hit, (path[i] - path[i - 1]).magnitude, lm)) {
                    return hit;
                }
            }
            return null;
